    ...
    fn visit_expr(&mut self, expr: &Expr) {
        ...
        if let Expr::Call(call) = expr {
            if let Expr::Attribute(outer) = call.func.as_ref() {
                if outer.attr.as_str() == "filter" || outer.attr.as_str() == "where" {
                    if let Expr::Call(inner_call) = outer.value.as_ref() {
                        if let Expr::Attribute(inner_attr) = inner_call.func.as_ref() {
                            if inner_attr.attr.as_str() == "select" {
                                self.violations.push(method_violation(
                                    outer, outer.attr.as_str(), self.source,
                                    self.file, self.index, self.severity, ID,
                                ));
                            }
                        }
                    }
