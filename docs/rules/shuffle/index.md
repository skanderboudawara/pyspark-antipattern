...
| [S003](S003.md) | `.groupBy()` followed by `.distinct()` or `.dropDuplicates()` |
...
| [S016](S016.md) | `first()` or `last()` with `.over(Window.partitionBy())` without `orderBy()` — non-deterministic result |
