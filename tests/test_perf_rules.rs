...
#[test]
fn perf001_fires_rdd_collect() {
    assert_violation(&check(perf001::check, "rows = df.rdd.collect()"), "PERF001", 1);
}
#[test]
fn perf001_fires_chain() {
    assert_violation(
        &check(perf001::check, "rows = df.filter(col('x') > 1).rdd.collect()"),
        "PERF001",
        1,
    );
}
...
#[test]
fn perf003_fires_over_threshold() {
...
#[test]
fn perf003_no_violation_under_threshold() {
...
#[test]
fn perf003_checkpoint_resets_counter() {
...
#[test]
fn perf003_fn_cost_no_violation_single_call() {
...
#[test]
fn perf003_fn_body_checked_independently() {
    // groupBy+agg=1, distinct=2, sort=3, repartition=4, join=5,
...
#[test]
fn perf003_standalone_agg_counts() {
...
#[test]
fn perf004_fires_bare_persist() {
    assert_violation(&check(perf004::check, "df.persist()"), "PERF004", 1);
}
#[test]
fn perf004_fires_chained() {
    assert_violation(
        &check(perf004::check, "df.filter(col('x') > 0).persist()"),
        "PERF004",
        1,
    );
}
#[test]
fn perf004_no_fire_memory_only() {
    assert_no_violation(
        &check(perf004::check, "df.persist(StorageLevel.MEMORY_ONLY)"),
        "PERF004",
    );
}
#[test]
fn perf004_no_fire_memory_and_disk() {
    assert_no_violation(
        &check(perf004::check, "df.persist(StorageLevel.MEMORY_AND_DISK)"),
        "PERF004",
    );
}
#[test]
fn perf004_no_fire_disk_only() {
    assert_no_violation(&check(perf004::check, "df.persist(StorageLevel.DISK_ONLY)"), "PERF004");
}
#[test]
fn perf004_no_fire_off_heap() {
...
#[test]
fn perf008_fires_inline_form() {
    assert_violation(
        &check(
            perf008::check,
            "df = spark.read.csv(spark.sparkContext.parallelize(data), header=True, sep=';')",
        ),
        "PERF008",
        1,
    );
...
#[test]
fn perf008_fires_sc_alias() {
    let src = "rdd = sc.parallelize(lines)\ndf = spark.read.csv(rdd, header=True)";
    assert_violation(&check(perf008::check, src), "PERF008", 2);
}
#[test]
fn perf008_no_csv_from_path() {
    assert_no_violation(
        &check(perf008::check, "df = spark.read.csv('/data/file.csv', header=True)"),
        "PERF008",
    );
}
#[test]
fn perf008_no_create_dataframe() {
    assert_no_violation(
        &check(
            perf008::check,
            "df = spark.createDataFrame(pd.read_csv(StringIO(data), sep=';'))",
        ),
        "PERF008",
    );
}
#[test]
fn perf008_no_parallelize_alone() {
    assert_no_violation(
